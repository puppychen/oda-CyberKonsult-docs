...
> **🔧 校正（2026-04-29，本判斷已撤回）**
>
> 經 PRD line 127-131「閾值語意說明」（2026-04-18 補強）確認，**0.005 與 0.003 為兩層不同語意閾值，並非矛盾**：
> - **過濾閾值**（AC-02-05-01）：`< 0.005`（RRF）→ 結果被**移除**；過濾後若無結果則保留 top 1 作為 fallback
> - **信心度判定閾值**（AC-23-01-04）：`< 0.003` → 對過濾後留存的 top 1 結果分級為 🔴 低信心度
>
> 兩層閾值在實作上對應 `chat.service.ts:326`（filterLowScoreResults）與 `chat.service.ts:386`（determineConfidenceLevel）兩個獨立函式。本段「矛盾 1」判斷係未閱讀 PRD line 127-131 註解所致，現撤回。
>
> 同步動作：PRD line 582 已補反向交叉註腳指向 line 127-131；RTM v1.5.1 記載此校正。
...
#### ~~❌ 飄移 #6：RRF 閾值不一致（未修復）~~ → 🔧 已撤回（2026-04-29）

~~位置：PRD.md:123 (`RRF < 0.005`) vs SRS_TECHNICAL.md:576 (`RRF < 0.003`)~~

> **校正**：詳見 §1.2「矛盾 1」段末校正聲明。0.005 與 0.003 為過濾 vs 信心度判定兩層不同語意閾值，非飄移；對應實作於 `chat.service.ts:326` 與 `chat.service.ts:386` 兩個獨立函式。本段判斷已撤回。
