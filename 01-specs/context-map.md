...
| Chat RAG | Conversations, messages, mode config, context building, LLM generation, feedback, confidence | `apps/api/src/modules/chat`, `apps/api/src/modules/llm`, `apps/api/src/modules/websearch`, `python/rag-service/src/rag_service/retrieval` | `RTM.md` FR-02/16/22/23, `../04-features/chat-rag/` | NestJS owns orchestration and user authorization; FastAPI owns retrieval. |
